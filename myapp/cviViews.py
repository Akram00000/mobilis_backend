...
class CVIDetailsAPIView(APIView):
    ...
    def get(self, request):
        ...
        for cvi in cvi_users:
            ...
            if zone:
                nb_visits = Visit.objects.filter(agent=cvi).count()
                nb_pdv_visited = Visit.objects.filter(agent=cvi).values('pdv').distinct().count()
                nb_pdv_zone = PointOfSale.objects.filter(zone=zone).count() 
            else:
                nb_visits = 0
                nb_pdv_visited = 0
                nb_pdv_zone = 0
            ...
            data.append({
                ...
                "nb_visits": nb_visits,
                "nb_pdv_visited": nb_pdv_visited,
                "nb_pdv_zone": nb_pdv_zone 
            })
...
class CVIProfileAPIView(APIView):
    ...
    def get(self, request, cvi_id):
        try:
            ...
            nb_pdv = Visit.objects.filter(agent__zone=zone).values('pdv').distinct().count() if zone else 0
    ...
    def get(self, request, cvi_id):
        try:
            ...
            for i in range(6):
                ...
                zone = cvi.zone
                goal_visits = PointOfSale.objects.filter(zone=zone).count() if zone else 0
...
class CVIVisitPerformanceAPIView(APIView):
    ...
    def get(self, request, cvi_id):
        try:
            ...
            zone = cvi.zone
            total_goal_pdv = PointOfSale.objects.filter(zone=zone).count() if zone else 0
