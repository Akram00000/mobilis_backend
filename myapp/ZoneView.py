...
from myapp.ZoningModel import merge_geojsons
from myapp.ZoneSerializer import ZoneSerializer
from myapp.models import Commune, PointOfSale, User, Wilaya
from rest_framework.permissions import IsAuthenticated
from myapp.zoning import assign_communes_from_geojson, create_balanced_zones, export_zones_to_geojson, generate_zone_boundaries, load_data
...
class GenerateZones(APIView):
    ...
    def post(self, request):
        ...
        memory_metrics = {"init": after_init_memory - start_memory}
        ...
        memory_metrics["after_csv_load"] = after_csv_memory - after_init_memory
        ...
        if not ('Commune' in df.columns and df['Commune'].notna().all()):
            df = assign_communes_from_geojson(df, 
                ...
                lat_col, lon_col)
        after_commune_memory = process.memory_info().rss / 1024 / 1024
        memory_metrics["after_commune_assign"] = after_commune_memory - after_csv_memory
        ...
        after_zones_memory = process.memory_info().rss / 1024 / 1024
        memory_metrics["after_zone_creation"] = after_zones_memory - after_commune_memory
        # Generate zone boundaries
        zone_polygons = generate_zone_boundaries(zones)
        after_boundaries_memory = process.memory_info().rss / 1024 / 1024
        memory_metrics["after_boundaries"] = after_boundaries_memory - after_zones_memory
        ...
        memory_metrics["total_memory_mb"] = total_memory
        memory_metrics["total_time_sec"] = total_time
